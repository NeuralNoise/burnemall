...
import beam.MyGeometry.collections.EdgeRing;
 ...
 * 
 * It can't be empty. It must be simple (accepting co�ncident edges to manage wholes). It must be counter clockwise.
 * 
			...
			for (Segment2D e2 : edges) {
				...
				if (e1.intersectAtSinglePoint(e2)) {
					...
					if (e1.hasPoint(intersection) || e2.hasPoint(intersection))
						continue; // TODO � voir si on autorise le cas des edges qui se touchent mais ne se croisent pas
									// (gestion des trous)
					else
						return false;
				}
